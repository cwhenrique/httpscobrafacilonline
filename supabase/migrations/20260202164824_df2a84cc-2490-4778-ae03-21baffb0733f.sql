-- Permitir que funcionário veja o CLIENTE quando ele criou um empréstimo para esse cliente
-- (resolve: empréstimos aparecem no contador, mas somem na lista porque loan.client vem null via RLS)

CREATE OR REPLACE FUNCTION public.can_view_client_optimized(
  _user_id uuid,
  _client_user_id uuid,
  _client_created_by uuid,
  _client_id uuid
)
RETURNS boolean
LANGUAGE plpgsql
...
BEGIN
  ...
  -- ✅ NOVO: Funcionário criou um empréstimo para este cliente?
  IF EXISTS (
    SELECT 1
    FROM public.loans l
...
-- Índice para acelerar a checagem acima em bases grandes
CREATE INDEX IF NOT EXISTS idx_loans_client_created_by_user
ON public.loans (client_id, created_by, user_id);
