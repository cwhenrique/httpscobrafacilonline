...
BEGIN
  ...
  -- Sum total penalty amounts from all payments (NEW: needed for condition 2 fix)
  SELECT COALESCE(SUM(
    COALESCE(
      (regexp_match(notes, '\[PENALTY_INCLUDED:([0-9.]+)\]'))[1]::numeric,
      0
    )
  ), 0) INTO total_penalty_amount
  FROM public.loan_payments
  WHERE loan_id = COALESCE(NEW.loan_id, OLD.loan_id)
    AND (notes NOT LIKE '%[PRE_RENEGOTIATION]%' OR notes IS NULL);
  ...
  IF GREATEST(0, total_to_receive - balance_reducing_payments) <= 0.01 THEN
    new_status := 'paid';
  -- FIX: Subtract penalty amounts from total_payments before comparing with total_to_receive
  -- This prevents penalties from inflating the total and prematurely closing contracts
