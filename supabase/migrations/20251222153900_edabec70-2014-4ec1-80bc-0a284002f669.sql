...
BEGIN
  ...
  ELSE
    ...
    -- Check if payment notes reference an installment (e.g., "Parcela 13 de 30" or "Parcelas 11, 12 de 20")
    IF OLD.notes IS NOT NULL AND OLD.notes ~ 'Parcela' THEN
      -- Get current loan notes
      SELECT notes INTO updated_notes FROM public.loans WHERE id = OLD.loan_id;
      
      IF updated_notes IS NOT NULL THEN
        -- Pattern for single installment: "Parcela X de Y"
        ...
        -- Pattern for multiple installments: "Parcelas X, Y, Z de N"
        IF OLD.notes ~ 'Parcelas [0-9]' THEN
          -- Extract all numbers before "de" and process each
          FOR num_text IN 
        ...
        -- Update loan notes with removed PARTIAL_PAID tags and clean up whitespace
        UPDATE public.loans
