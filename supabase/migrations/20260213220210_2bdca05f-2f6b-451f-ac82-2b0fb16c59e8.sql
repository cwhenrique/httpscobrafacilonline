...
BEGIN
  ...
  -- In this case, set remaining_balance = 0 and status = paid, regardless of payment sums
  IF loan_notes LIKE '%[DISCOUNT_SETTLEMENT:%' THEN
    -- Extract discount value for reference
    discount_match := regexp_match(loan_notes, '\[DISCOUNT_SETTLEMENT:([0-9.]+)\]');
