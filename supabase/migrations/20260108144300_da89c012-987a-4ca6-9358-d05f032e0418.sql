-- Atualizar função recalculate_loan_total_paid para excluir pagamentos de juros do cálculo do remaining_balance
...
BEGIN
  ...
  -- Soma total de todos os pagamentos (para total_paid)
  SELECT COALESCE(SUM(amount), 0) INTO actual_total
  FROM public.loan_payments
  WHERE loan_id = COALESCE(NEW.loan_id, OLD.loan_id);
  
  -- Soma de pagamentos que REDUZEM o saldo (exclui INTEREST_ONLY_PAYMENT)
  ...
  IF loan_record.notes IS NOT NULL THEN
    ...
    FROM regexp_matches(loan_record.notes, '\[OVERDUE_CONSOLIDATED:([0-9.]+):[^\]]+\]', 'g') m;
  END IF;
...
-- Correção em massa: recalcular remaining_balance de empréstimos com pagamentos de juros
WITH loan_calculations AS (
  SELECT 
    ...
    ) as balance_reducing_paid
  FROM loans l
  WHERE l.status != 'paid'
)
UPDATE loans 
SET remaining_balance = GREATEST(0, 
  lc.base_total + lc.penalties + lc.consolidated - lc.balance_reducing_paid - lc.discount
)
