...
-- Corrige contratos marcados como 'overdue' incorretamente quando due_date é no futuro
-- E também corrige contratos já pagos (remaining_balance <= 0) que estão como overdue

-- 1. Corrigir contratos com due_date no futuro que não deveriam estar em atraso
UPDATE public.loans
SET status = 'pending'
...
-- 2. Corrigir contratos já pagos (remaining_balance <= 0) que ainda estão como overdue
UPDATE public.loans
SET status = 'paid'
WHERE status = 'overdue' 
  AND remaining_balance <= 0;

-- 3. Atualizar o trigger para verificar corretamente a data de vencimento
...
-- 4. Atualizar o trigger para reverter pagamentos corretamente
